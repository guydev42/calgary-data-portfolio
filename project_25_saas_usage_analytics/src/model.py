"""
Train, evaluate, and explain SaaS churn prediction models.
Includes: Logistic Regression, Random Forest, XGBoost, LightGBM.
Generates SHAP explanations and at-risk account analysis.
"""
...
def train_and_evaluate(X_train, X_test, y_train, y_test, feature_names):
    ...
    and compute at-risk account analysis. Saves the best model to disk.
    """
    _ensure_dirs()
    ...
    _at_risk_analysis(best_probs, y_test, best_name)
...
def _generate_shap(trained_models, feature_names, X_test, X_test_scaled, best_name):
    ...
    # Waterfall plot for a single user (first predicted churner)
    try:
...
def _at_risk_analysis(y_prob, y_test, model_name):
    """
    At-risk account analysis for SaaS churn intervention.

    Assumptions:
    - Customer success outreach cost: $80 per account
    - Average annual contract value (ACV) of a churned account: $3,600
    - Retention success rate: 35% of contacted at-risk accounts are retained
    """
    print("\n" + "=" * 70)
    print("AT-RISK ACCOUNT ANALYSIS")
    print("=" * 70)
    ...
    for t in thresholds:
        ...
        total_outreach = tp + fp
        cost_of_outreach = total_outreach * outreach_cost
        retained_accounts = tp * retention_rate
        revenue_saved = retained_accounts * acv_churned
        net_savings = revenue_saved - cost_of_outreach
        ...
        impact_records.append({
            ...
            "outreach_cost": cost_of_outreach,
    ...
    print(f"\nModel: {model_name}")
    print(f"Outreach cost per account:   ${outreach_cost}")
    print(f"Average annual contract value: ${acv_churned}")
    ...
    print(f"  Total outreach:     {int(optimal['outreach_count'])}")
    print(f"  Outreach cost:      ${int(optimal['outreach_cost']):,}")
    print(f"  Revenue saved:      ${int(optimal['revenue_saved']):,}")
    print(f"  NET SAVINGS:        ${int(optimal['net_savings']):,}")

    # Scale to full user base (test set is 20% of 3000)
    scale_factor = 5
    annual_savings = int(optimal["net_savings"]) * scale_factor
    print(f"  Full base estimate: ${annual_savings:,}/year (scaled from test set)")

    # Save impact data
    impact_df.to_csv(os.path.join(OUTPUTS_DIR, "at_risk_analysis.csv"), index=False)
    ...
    plt.title(f"At-risk account analysis - {model_name}")
    fig.tight_layout()
    fig.savefig(os.path.join(OUTPUTS_DIR, "at_risk_analysis.png"), dpi=150)
    plt.close(fig)
    print("Saved at-risk analysis plot.")
