"""Tests for project_24 model module."""
...
def _make_synthetic_data():
    ...
    n_train, n_test, n_features = 200, 50, 15
    X_train = rng.randn(n_train, n_features)
    y_train = rng.choice([0, 1], n_train, p=[0.7, 0.3])
    X_test = rng.randn(n_test, n_features)
    y_test = rng.choice([0, 1], n_test, p=[0.7, 0.3])
...
def test_random_forest_trains():
    """Random Forest from the model config should fit and predict."""
    models = _get_models_and_grids()
    X_train, X_test, y_train, y_test, _ = _make_synthetic_data()
