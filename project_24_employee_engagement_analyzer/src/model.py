"""
Train, evaluate, and explain employee disengagement prediction models.
...
def _generate_shap(trained_models, feature_names, X_test, X_test_scaled, best_name):
    ...
    plt.title("SHAP feature impact on disengagement prediction")
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUTS_DIR, "shap_summary.png"), dpi=150, bbox_inches="tight")
    plt.close("all")
    print("Saved SHAP summary plot.")

    # Waterfall plot for a single employee (first predicted disengaged)
    try:
        probs = model.predict_proba(X_sample)[:, 1]
        disengage_idx = np.where(probs > 0.5)[0]
        if len(disengage_idx) > 0:
            single_idx = disengage_idx[0]
        else:
            single_idx = 0
        ...
        plt.title("SHAP waterfall - single employee prediction")
        plt.tight_layout()
...
def _business_impact(y_prob, y_test, model_name):
    """
    Cost-benefit analysis for disengagement intervention.

    Assumptions:
    - Retention intervention cost: $500 per employee (coaching, development plan)
    - Average replacement cost of a disengaged employee who leaves: $15,000
    - Retention success rate: 40% of contacted disengaged employees re-engage
    - If we catch a true disengaged employee AND retain them (40% chance), we save $15,000
    - If we intervene on an engaged employee (false positive), we waste $500
    - Missed disengaged employees cost the full replacement cost
    """
    print("\n" + "=" * 70)
    print("BUSINESS IMPACT ANALYSIS")
    print("=" * 70)

    intervention_cost = 500      # cost per employee intervention
    replacement_cost = 15000     # cost to replace a disengaged employee who leaves
    retention_rate = 0.40        # 40% of intervened disengaged employees re-engage
    ...
    for t in thresholds:
        ...
        retained_employees = tp * retention_rate
        revenue_saved = retained_employees * replacement_cost
        net_savings = revenue_saved - cost_of_interventions
        disengaged_caught_pct = tp / (tp + fn) * 100 if (tp + fn) > 0 else 0
        ...
        impact_records.append({
            ...
            "disengaged_caught_pct": round(disengaged_caught_pct, 1),
        })
    ...
    print(f"\nModel: {model_name}")
    print(f"Intervention cost per employee:    ${intervention_cost}")
    print(f"Employee replacement cost:          ${replacement_cost}")
    print(f"Re-engagement success rate:         {retention_rate:.0%}")
    print(f"\nOptimal threshold: {optimal['threshold']:.3f}")
    print(f"  Disengaged caught:   {int(optimal['true_positives'])} / {int(optimal['true_positives'] + optimal['false_negatives'])} ({optimal['disengaged_caught_pct']:.1f}%)")
    ...
    # Scale to full employee base (test set is 20% of 8000)
    scale_factor = 5
    ...
    ax2 = ax1.twinx()
    ax2.plot(impact_df["threshold"], impact_df["disengaged_caught_pct"], "g--s", markersize=4, alpha=0.7, label="Disengaged caught (%)")
    ax2.set_ylabel("Disengaged caught (%)", color="g")
